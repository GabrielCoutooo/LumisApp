...
function corPorPercentual(percentual) {
  if (percentual >= 0 && percentual <= 60) return "#3B82F6";
            ...
            <div class="orcamento-card">
                <div class="orcamento-header">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <div class="progress-text" style="color:${cor};font-weight:bold;">${percentual}%</div>
                    </div>
                </div>
                <div class="orcamento-values">${formatarMoeda(
                  gasto
...
function prepararTelaRegistro() {
  ...
  document.getElementById("input-efetuada").checked = true;
...
async function exportarDados() {
  try {
    const url = `${BASE_API}/user/exportar?id_usuario=${ID_USUARIO}&formato=csv`;
    window.open(url, "_blank");
    mostrarNotificacao(
      "info",
      "Exportando...",
      "O download do arquivo CSV será iniciado"
    );
  } catch (error) {
  ...
  try {
    ...
    lista.innerHTML = dados
      ...
      <div class="item-gerenciar">
        ...
        <div class="item-gerenciar-acoes">
          <button class="btn-icon" onclick="alert('Editar categoria em desenvolvimento')">
            <i class="fa-solid fa-pen"></i>
          </button>
          <button class="btn-icon btn-icon-danger" onclick="alert('Excluir categoria em desenvolvimento')">
            <i class="fa-solid fa-trash"></i>
          </button>
        </div>
      </div>
